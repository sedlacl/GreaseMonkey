...
// @updateURL    https://raw.githubusercontent.com/sedlacl/GreaseMonkey/refs/heads/main/message-registry-preview-downloads.uucloud1.user.js
// @downloadURL  https://raw.githubusercontent.com/sedlacl/GreaseMonkey/refs/heads/main/message-registry-preview-downloads.uucloud1.user.js
// ==/UserScript==
...
(function () {
  ...
  function getControlLabel(control) {
    ...
    return [control.getAttribute("aria-label"), control.getAttribute("title"), control.getAttribute("data-testid"), control.textContent]
      .filter(Boolean)
      .join(" ");
  }

  function hasDownloadIcon(control) {
    return Boolean(control?.querySelector?.(".uugds-download, .mdi-download, [class*='mdi-download'], [class*='download']"));
  }
  ...
  function looksLikeDownloadControl(control) {
    ...
    const label = normalizeLabel(getControlLabel(control));
    return label.includes("download") || label.includes("stahn") || label.includes("payload") || hasDownloadIcon(control);
  }

  function detectPayloadTypeFromControl(control) {
    const label = normalizeLabel(getControlLabel(control));
    if (label.includes("external") || label.includes("extern")) {
      return "external";
    }
    if (label.includes("internal") || label.includes("intern")) {
      return "internal";
    }
    return null;
  }

  function getPayloadButtons() {
    const items = [];
    const seen = new Set();

    const addItem = (control, payloadType) => {
      if (!(control instanceof HTMLElement) || !payloadType || seen.has(control) || control.classList.contains(PREVIEW_BUTTON_CLASS)) {
        return;
      }
      if (control.closest("tr")) {
        return;
      }

      seen.add(control);
      items.push({ control, payloadType });
    };

    document.querySelectorAll(PAYLOAD_BUTTON_SELECTOR).forEach((control) => {
      addItem(control, detectPayloadTypeFromControl(control));
    });

    document.querySelectorAll(LEGACY_ACTION_SELECTOR).forEach((control) => {
      if (!(control instanceof HTMLElement) || !looksLikeDownloadControl(control)) {
        return;
      }
  ...
  function findPayloadButton(payloadType) {
    return getPayloadButtons().find((item) => item.payloadType === payloadType)?.control || null;
  }

  function findBusinessMetadataHeader() {
    const modernHeader = Array.from(document.querySelectorAll('[data-testid="page-header-text"]')).find((node) => {
      return BUSINESS_METADATA_LABELS.includes((node.textContent || "").trim());
    });
    if (modernHeader instanceof HTMLElement) {
      return modernHeader;
    }

    return Array.from(document.querySelectorAll("span, div, h1, h2, h3, h4, h5, h6")).find((node) => {
      return BUSINESS_METADATA_LABELS.includes((node.textContent || "").trim()) && !node.closest(`.${PREVIEW_BUTTON_CLASS}`);
    });
  }

  function getPreviewInsertAnchor(control) {
    if (!(control instanceof HTMLElement)) {
      return control;
    }

    const wrapperLink = control.closest("a");
    if (wrapperLink instanceof HTMLAnchorElement && !wrapperLink.closest("tr")) {
      return wrapperLink;
    }
  ...
  function shouldMirrorAnchorLayout(insertAnchor) {
    return insertAnchor instanceof HTMLAnchorElement && !insertAnchor.closest("tr");
  }

  function getAttachmentLink(target) {
    const row = target.closest?.("tr") || (target instanceof HTMLTableRowElement ? target : null);
    if (!row) return false;

    const directLink = target.closest?.("a");
    if (directLink instanceof HTMLAnchorElement) {
      const directButton = directLink.querySelector("button") || row.querySelector("button");
      if (looksLikeDownloadControl(directLink) || hasDownloadIcon(directLink) || (directButton && looksLikeDownloadControl(directButton))) {
        return directLink;
      }
    }
    ...
    return hasDownloadIcon(link) || hasDownloadIcon(button) || looksLikeDownloadControl(button) ? link : null;
  }
  ...
  function getAccessTokenFromSessionStorage() {
    for (let index = 0; index < window.sessionStorage.length; index += 1) {
      ...
      if (!key || !key.startsWith("uu_app_oidc_providers_")) {
        continue;
      }
      ...
      try {
        ...
        const sessionValue = JSON.parse(rawValue);
        const accessToken = sessionValue?.accessToken || sessionValue?.acccessToken;
  ...
  function fetchWithCurrentAuth(url, init = {}) {
  ...
  async function previewPayloadDirectly(payloadType) {
    ...
    try {
      const response = await fetchWithCurrentAuth(requestUrl, {
        headers: {
          Accept: "*/*",
        },
      });
  ...
  function createPreviewButton(referenceButton, options) {
    ...
    const className = typeof referenceButton.className === "string" ? referenceButton.className : "";
    button.classList.add(...className.split(/\s+/u).filter(Boolean));
    ...
    if (options.alignWithAnchorLayout) {
  ...
  function ensurePayloadPreviewButtons() {
    ...
    getPayloadButtons().forEach(({ control, payloadType }) => {
      const insertAnchor = getPreviewInsertAnchor(control);
      const existingButton = insertAnchor?.nextElementSibling;
      ...
      insertAnchor.insertAdjacentElement(
        "afterend",
        createPreviewButton(control, {
          kind: "payload",
          payloadType,
          alignWithAnchorLayout: shouldMirrorAnchorLayout(insertAnchor),
  ...
  function ensureMessageSourceButton() {
    ...
    const internalButton = findPayloadButton("internal");
    if (!(internalButton instanceof HTMLElement)) {
      return;
    }

    const internalInsertAnchor = getPreviewInsertAnchor(internalButton);
    const payloadPreviewButton =
      internalInsertAnchor?.nextElementSibling?.classList?.contains(PREVIEW_BUTTON_CLASS) &&
      internalInsertAnchor.nextElementSibling.dataset.previewKind === "payload"
        ? internalInsertAnchor.nextElementSibling
        : null;
    const anchorElement = payloadPreviewButton || internalInsertAnchor;
    ...
    anchorElement.insertAdjacentElement(
      ...
      createPreviewButton(internalButton, {
        kind: "source",
        alignWithAnchorLayout: shouldMirrorAnchorLayout(anchorElement),
  ...
  function ensureChannelMetadataButton() {
    ...
    const headerText = findBusinessMetadataHeader();
  ...
  function ensureAttachmentPreviewButtons() {
    ...
    document.querySelectorAll("tr").forEach((row) => {
      const attachmentLink = getAttachmentLink(row);
      if (!attachmentLink) {
        return;
      }
